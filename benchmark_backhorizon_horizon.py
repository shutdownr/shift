...
from basic_models import (
    ...
    predict_naive,
    predict_nbeats,
    predict_nonstationary_transformer,
    predict_patchtst,
    predict_shift,
    predict_theta,
    predict_timemixer,
...
algorithms = {
    ...
    "iTransformer": predict_itransformer,
...
def benchmark(horizon, backhorizon, algorithms, datasets, error_metrics, random_states):
    ...
    for dataset_name in datasets:
        dataset = read_dataset(
            ...
            backhorizon,
            horizon,
            max_series=Config.max_series[dataset_name],
        )
        ...
        for algo_name, algorithm in algorithms.items():
            ...
            if algo_name == "SHIFT":
                ...
                y_pred, time = algorithm(
                    dataset_scaled_flat,
                    Config.timesnet_d_ff[dataset_name],
                    Config.timesnet_d_model[dataset_name],
                    random_states,
                )
                y_pred = [
                    inverse_flatten_predictions(pred, original_shapes)
                    for pred in y_pred
                ]
                y_pred = np.array(
                    [inverse_scale_predictions(pred, scalers) for pred in y_pred]
                )
            ...
            elif algo_name in ["Chronos2", "Moirai"]:
                # Zero-shot foundation models: must receive raw (unscaled) data —
                # they perform their own internal normalization and are confused by
                # MinMax-scaled [0,1] inputs.
                y_pred, time = algorithm(dataset_flat, random_states)
                y_pred = [
                    inverse_flatten_predictions(pred, original_shapes)
                    for pred in y_pred
                ]
...
if __name__ == "__main__":
    ...
    for i, h in enumerate(H):
        for j, bh in enumerate(BH):
            errors, inference_times = benchmark(
                h, bh, algorithms, dataset_names, error_metrics, random_states
            )
